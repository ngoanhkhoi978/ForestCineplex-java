...
public class AddUser extends javax.swing.JPanel {
    ...
    public AddUser(Management management) {
    ...
    public void CheckAddUser() {
        ...
        try (ResultSet resultSet = DataConnection.executeQuery("SELECT * FROM user WHERE User = ?", username)) {
            if (resultSet != null && resultSet.next()) {
                jTextFielUsername.setText("");
                jTextFielUsername.setBorder(errorBorder);
                jLabelError.setText("The username has already been registered");
                return;
            }
        ...
        User user = new User(username, passwordHash, Role, fullname, Gender, Double.valueOf(Salary), Address, Phone, Mail, birthDate);
        Add(user);
        setDefault();
    ...
    public void Add(User user) {
        ...
        String[] columns = {"User", "PasswordHash", "Role", "FullName", "Gender", "BirthDate", "Address", "Phone", "Mail", "Salary"};
        String[] values = {user.getUser(), user.getPasswordHash(), user.getRole(), user.getFullname(), user.getGender(), day, user.getAddress(), user.getPhone(), user.getMail(), String.valueOf(user.getSalary())};
        DataConnection.insertData("user", columns, values);
    }
